...
test("RpcProcess forwards the configured omp workflow file", async () => {
  const previous = process.env.OMP_WEB_OMP_CONFIG;
  process.env.OMP_WEB_OMP_CONFIG = "D:\\workflow\\engineering-workflow.yml";
  const transport = makeTransport();
  try {
    const proc = startProcess(transport);
    assert.deepEqual(transport.spawnCalls[0][1].slice(-2), ["--config", "D:\\workflow\\engineering-workflow.yml"]);
    await proc.dispose(0);
  } finally {
    if (previous === undefined) delete process.env.OMP_WEB_OMP_CONFIG;
    else process.env.OMP_WEB_OMP_CONFIG = previous;
  }
});

test("all session children inherit one workflow config while keeping distinct cwds", async () => {
  const previous = process.env.OMP_WEB_OMP_CONFIG;
  process.env.OMP_WEB_OMP_CONFIG = "D:\\workflow\\engineering-workflow.yml";
  ...
  try {
    const first = new RpcProcess({
      cwd: "D:\\projects\\one",
      dependencies: { resolveOmpBin: () => "fake-omp", spawn: firstTransport.spawn },
    });
    const second = new RpcProcess({
      cwd: "D:\\projects\\two",
      dependencies: { resolveOmpBin: () => "fake-omp", spawn: secondTransport.spawn },
    });
    assert.deepEqual(firstTransport.spawnCalls[0][1], [
      "--mode", "rpc-ui", "--cwd", "D:\\projects\\one",
      "--config", "D:\\workflow\\engineering-workflow.yml",
    ]);
    assert.deepEqual(secondTransport.spawnCalls[0][1], [
      "--mode", "rpc-ui", "--cwd", "D:\\projects\\two",
      "--config", "D:\\workflow\\engineering-workflow.yml",
    ]);
    await Promise.all([first.dispose(0), second.dispose(0)]);
  } finally {
    if (previous === undefined) delete process.env.OMP_WEB_OMP_CONFIG;
    else process.env.OMP_WEB_OMP_CONFIG = previous;
  }
